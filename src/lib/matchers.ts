...
import { MarbleDefaultError } from "../models/errors/marbledefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | MarbleDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | MarbleDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new MarbleDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new MarbleDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
